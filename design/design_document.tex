...
In this project we design and implement a software calles Cryptocast. CryptoCast is a piece of software that provides a service to
...
When starting the client app the first screen is the Homescreen. If the client already connected to a server and that server was saved, 
the host panel is already filled out. If not, the user has to enter a host address. Once an address is entered, the connect button is enabled.
When the connect button is pressed, the client tries to connect to the server. If it fails there is an error popup and the program goes back to 
Homescreen. If the host address was correct and the client is able to connect, the program checks if it already connected to this server. If it 
did, it already has the key and goes directly to the stream viewer. If the key file is missing, the file browser pops up and the user has to select 
the necessary key. If the key is correct, the user is able to view the stream. Facing a problem like disconnection, the client shows an error and 

...
public class Doclet {
    ...
    public static JSONObject classInfo(ClassDoc cls) throws java.io.IOException, RecognitionException {
        ...
        class_info.put("abstract", cls.isAbstract());
        class_info.put("interface", !cls.isClass());
        /// @end
        ...
        for (int j = 0; j < fields.length; j++) {
            ...
            addMemberInfo(fields[j], field_info);
        }
    ...
    public static JSONObject getParamInfo(ExecutableMemberDoc doc) throws java.io.IOException {
        JSONObject parameters = new JSONObject();
        for (Parameter p : doc.parameters()) {
            JSONObject param_info = new JSONObject();
            for (ParamTag ptag : doc.paramTags()) {
                if (ptag.parameterName().equals(p.name()))
                    param_info.put("comment", ptag.parameterComment());
            }
            param_info.put("simple-type", p.type().simpleTypeName() + p.type().dimension());
            param_info.put("type", p.type().qualifiedTypeName());
            parameters.put(p.name(), param_info);
        }
        return parameters;
    }

    public static void addMemberInfo(MemberDoc doc, JSONObject obj) throws java.io.IOException {
        obj.put("visibility", "private");
        if (doc.isProtected())
            obj.put("visibility", "protected");
        else if (doc.isPublic())
            obj.put("visibility", "public");
        obj.put("static", doc.isStatic());
        obj.put("final", doc.isFinal());
    }
    ...
    public static JSONObject constructorInfo(ConstructorDoc constructor) throws java.io.IOException {
        ...
        constructor_info.put("parameters", getParamInfo(constructor));
        addMemberInfo(constructor, constructor_info);
    ...
    public static JSONObject methodInfo(MethodDoc method) throws java.io.IOException {
        ...
        method_info.put("parameters", getParamInfo(method));
        method_info.put("abstract", method.isAbstract());
        addMemberInfo(method, method_info);

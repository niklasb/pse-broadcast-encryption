...
\umlstatic{-- printCommands()} \\
\umlstatic{+ main(argv : String[])} \\
\umlstatic{-- runBenchmark(benchmark : Benchmark, rep : int, freq : int)} \\
\umlstatic{-- measure(benchmark : Benchmark, rep : int, freq : int) : Result} \\
\umlstatic{-- makePrime(b : int) : BigInteger}
}
\end{tikzpicture}
\vspace{0.3em}
\end{minipage}

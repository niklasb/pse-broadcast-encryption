...
\begin{itemize}
   \item sendet Daten an eine Empfängergruppe
   \item erlaubt es, Schlüssel zu generieren, die dann an Clients verteilt werden
   \item verschlüsselt Daten
...
\begin{itemize}
   ...
   \item empfängt Daten vom Server
   \item entschl"usselt Daten vom Server mithilfe eines konfigurierbaren Schl"ussels
   \item merkt sich zuletzt ausgew"ahlte Server, sodass ein erneuter Zugriff schnell
         m"oglich ist
\end{itemize}

\subsubsection{Kommunikation und Verschlüsselung}
\begin{itemize}
   \item Das verwendete Broadcast"=Verschl"usselungsverfahren basiert auf
         dem in~\cite[Section~2.2]{Naor00} beschriebenen Algorithmus
\end{itemize}
